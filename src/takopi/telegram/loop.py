...
from ..swarm.config import resolve_swarm_ingress_config
from ..swarm.inbox import poll_swarm_inbox
...
async def _forward_updates(
    updates: AsyncIterator[TelegramIncomingUpdate],
    sender,
) -> None:
    async with sender:
...
async def run_main_loop(
    ...
    *,
    ...
    try:
        ...
        swarm_ingress = resolve_swarm_ingress_config(cfg.runtime)
        if swarm_ingress is not None:
            logger.info(
                "swarm.ingress.enabled",
                inbox_path=str(swarm_ingress.inbox_path),
                poll_interval_s=swarm_ingress.poll_interval_s,
            )
        ...
        async with anyio.create_task_group() as tg:
            ...
            async def route_message(msg: TelegramIncomingMessage) -> None:
                ...
                is_swarm_trigger = (
                    msg.ingress_source == "swarm" and msg.ingress_intent == "trigger"
                )
                if (
                    trigger_mode == "mentions"
                    and not is_swarm_trigger
            ...
            async def route_update(update: TelegramIncomingUpdate) -> None:
                bypass_allowed_users = (
                    isinstance(update, TelegramIncomingMessage)
                    and update.ingress_source == "swarm"
                )
                if allowed_user_ids and not bypass_allowed_users:
            ...
            if swarm_ingress is None:
                ...
                poller_sender = send_stream.clone()
                swarm_sender = send_stream.clone()
                await send_stream.aclose()
                tg.start_soon(_forward_updates, poller_fn(cfg), poller_sender)
                tg.start_soon(
                    _forward_updates,
                    poll_swarm_inbox(swarm_ingress, sleep=sleep),
                    swarm_sender,
                )
                async with recv_stream:
                    async for update in recv_stream:
                        await route_update(update)
    finally:
