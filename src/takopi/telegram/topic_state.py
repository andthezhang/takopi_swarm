...
def _parse_thread_key(raw: str) -> tuple[int, int] | None:
    try:
...
class TopicStateStore(JsonStateStore[_TopicState]):
    ...
    ) -> list[TopicThreadSnapshot]:
        async with self._lock:
            self._reload_locked_if_needed()
            snapshots: list[TopicThreadSnapshot] = []
            for raw_key, thread in self._state.threads.items():
                parsed = _parse_thread_key(raw_key)
                if parsed is None:
                    continue
                item_chat_id, item_thread_id = parsed
                if chat_id is not None and item_chat_id != chat_id:
                    continue
                snapshots.append(
                    self._snapshot_locked(thread, item_chat_id, item_thread_id)
                )
            snapshots.sort(key=lambda item: (item.chat_id, item.thread_id))
            return snapshots
